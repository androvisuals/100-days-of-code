...
class SaveGuiState
{
	...
	String KeyWeb = "KEY_WEB";
	String KeyDouble = "0";
	String KeyBoolean = "KEY_BOOLEAN";
	Preferences state = Preferences.userRoot().node("javaFX gui prefs");

	String prefWeb = state.get(KeyWeb, "");
	
	double storedDouble = state.getDouble(KeyDouble, 0);
	boolean prefBoolean = state.getBoolean(KeyBoolean, false);
...
//	preferences.putBoolean(KeyBoolean, true);
	void setSliderValue(int setSlider)
	{
		value = setSlider;
	}
